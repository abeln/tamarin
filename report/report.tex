...
Notice how memory is also immutable: every \textsf{store} (\lstinline{sw}) operation returns a new array (memory), and every \textsf{select} (\lstinline{lw}) uses the latest memory constant .
...
\begin{itemize}
\item \cite{hawblitzel2013will} is probably the most relevant reference. They verify compiler correctness by checking program equivalence of emitted assembly code. They use x86, Boogie, and SymDiff, much in the same way we would like to do for this project.
\item \cite{lahiri2012symdiff} is also very relevant. They built SymDiff, a tool built on top of Boogie to test for program equivalence. It is unclear if we will be able to use it, though, since the released repository seems somewhat abandoned.
\item \cite{barnett2005boogie} present Boogie, an intermediate verification language that can be targeted as an IR for program verification tasks.
\item \cite{ramos2011practical} present UC-KLEE, which given two C functions checks whether they are equivalent (up to a fixed input size of 8 bytes). Their notion of equivalence considers not only function return values, but also memory locations reachable from them. UC-KLEE is built on top of KLEE. As far as we can tell, UC-KLEE was never released publicly.
\item \cite{cadar2008klee} is another popular ``backend'' for program verification tasks. It implements a symbolic VM for LLVM bitcode.
\item \cite{egele2014blanket} use \emph{Blanket Execution} to identify portions of program binaries that are ``'similar', based on randomized testing and having similar side-effects.
\item \cite{partush2013abstract} use abstract interpretation to prove program equivalence for numerical programs.
\item \cite{yang2014property} show how verify that manually-annotated program properties continue to hold as a program evolves.
\item \cite{necula2000translation} verifies equivalence of IRs before and after compiler passes.
\item \cite{francesco2014grease} present GreASE, which focuses on non-equivalence checking.
\item \cite{sharma2013data} are able to check equivalence of x86 loops, unlike most of the other tools, which need to unroll loops.
\item \cite{person2011directed} combine static analysis and symbolic execution for program equivalence.
\end{itemize}
\section{Conclusions}
