...
We can now define a relation $\erel \subseteq P \times P$ of equivalent programs. Given $P_1, P_2 \in P$, we say that $\pequiv{P_1}{P_2}$ (read ``$P_1$ is equivalent to $P_2$'') if, for all inputs $\$1$ and $\$2$, one of the following holds:
\begin{itemize}
...
To get decidability back, we define a new class of relations $\merel{S} \subseteq P \times P$. We say that $\pmequiv{P_1}{P_2}{S}$ (read ``$P_1$ is $S$-equivalent to $P_2$'') if, for all inputs, one of the following holds:
\begin{itemize}
...
Corollary \ref{soundness} can be used to argue the soundness (with respect to $\erel$) of any decision procedure that over-approximates $\merel{S}$. In the next section we will show one such over-approximation based on concolic execution.
