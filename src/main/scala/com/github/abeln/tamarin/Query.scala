...
object Query {
  ...
  private case class Ctx(z3: Context, bv32: BitVecSort, bv64: BitVecSort)
  ...
  private def mkConsts(trace: Trace)(implicit c: Ctx): Map[Reg, BitVecExpr] = {
    val Ctx(ctx, bv32, bv64) = c
  ...
  private def mkAsserts(trace: Trace, regMap: Map[Reg, BitVecExpr])(implicit c: Ctx): Seq[BoolExpr] = {
    val Ctx(ctx, bv32, bv64) = c
